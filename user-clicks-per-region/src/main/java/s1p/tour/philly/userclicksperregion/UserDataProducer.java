...
public class UserDataProducer {
	...
	public static void main(String... args) throws Exception{
		...
		generateRegionsData(props, userRegions);

		//Generate initial clicks data
		...
		generateClicksData(props, userClicks);

		//Wait for a few seconds to simulate user changing the region
		Thread.sleep(5000L);

		List<KeyValue<String, String>> modifiedUserRegions = Arrays.asList(
				new KeyValue<>("alice", "asia")   /* Alice back to Asia... */
		);

		generateRegionsData(props, modifiedUserRegions);

		List<KeyValue<String, Long>> moreUserClicks = Arrays.asList(
				new KeyValue<>("alice", 13L)
		);

		generateClicksData(props, moreUserClicks);

	}

	private static void generateClicksData(Map<String, Object> props, List<KeyValue<String, Long>> userClicks) {
	...
	private static void generateRegionsData(Map<String, Object> props, List<KeyValue<String, String>> userRegions) {
		props.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
